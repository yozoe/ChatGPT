...
| 能力 | 已确认内容 | 尚待确认 |
| --- | --- | --- |
| 文件和文件夹 | 官方文档确认 `@` 可搜索工作区文件并把路径加入提示；App Server 支持 `localImage`，Skill 使用结构化输入 | 桌面端是否使用文件搜索菜单、系统选择器或两者；普通文件是否存在结构化输入 |
...
| 录制技能 | 客户端存在技能创建/录制相关产品入口；尚无证据证明本地“附加 skill-creator + 提示词”完全等价 | 精确名称、采集过程、保存与失败生命周期 |
