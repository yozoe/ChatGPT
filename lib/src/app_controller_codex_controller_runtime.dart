...
import 'app_controller_thread_view_snapshot.dart';
import 'app_controller_workspace_task_list.dart';
...
import 'app_controller_runtime_diagnostics.dart';
import 'app_controller_attachments.dart';
import 'app_controller_workspace_paths.dart';
import 'app_controller_timeline_entries.dart';
...
class CodexController extends ChangeNotifier {
  ...
  CodexController({
    ...
    _gitOperations = CodexGitOperations(
      ...
      nextProjectRequest: () => ++_gitProjectRefreshRequest,
      nextDiffRequest: () => ++_gitDiffRefreshRequest,
      nextReviewRequest: () => ++_gitReviewRefreshRequest,
      ...
      setDiffChange: (value) => gitDiffChange = value,
      setDiff: (value) => gitDiff = value,
      setDiffLoading: (value) => gitDiffLoading = value,
      setDiffTruncated: (value) => gitDiffTruncated = value,
      setReviewDiffs: (value) => gitReviewDiffs = value,
      setReviewDiffErrors: (value) => gitReviewDiffErrors = value,
      setReviewLoading: (value) => gitReviewLoading = value,
      isOperationRunning: () => gitOperationRunning,
      setOperationRunning: (value) => gitOperationRunning = value,
    ...
    _attachments = CodexAttachmentCoordinator(
      store: _conversationAttachmentStore,
      temporaryPaths: _temporaryAttachmentPaths,
      composerRetains: _composerTemporaryAttachmentRetains,
      isReferenced: isAttachmentPathReferenced,
      isClipboardTemporaryPath: _isClipboardTemporaryPath,
    );
    _workspacePaths = CodexWorkspacePaths();
    _timelineEntries = CodexTimelineEntryFactory();
  ...
  late final CodexWorkspacePaths _workspacePaths;
  late final CodexTimelineEntryFactory _timelineEntries;
  ...
  Future<void> retryThreadWriterConflict() async {
    ...
    try {
      switch (conflict.operation) {
        case ThreadWriterConflictOperation.resume:
          await resumeThread(conflict.threads.single);
        case ThreadWriterConflictOperation.archive:
  ...
  void _setThreadWriterConflict({
    ...
    required ThreadWriterConflictOperation operation,
  }) {
    ...
    _threadWriterConflict = ThreadWriterConflict(
      workspace: workspace,
  ...
  void _setArchivedThreadRestore(CodexThread thread) {
    ...
    _archivedThreadRestore = ArchivedThreadRestore(
      workspace: workspace,
      thread: thread,
    );
  }
  ...
  ) => _attachments.discardUnreferenced(paths);
  ...
  Future<void> _reclaimUnreferencedThreadAttachments(Iterable<String> paths) =>
      _attachments.reclaimUnreferenced(paths);
  ...
  void transferTemporaryAttachmentTo(String path, CodexController target) {
    _attachments.transferTo(path, target._attachments);
  }
  ...
  void releaseDetachedTemporaryAttachments() {
    _attachments.releaseDetached();
  }

  void _releaseAllTemporaryAttachments() {
    _attachments.releaseAll();
  }
  ...
  Future<void> refreshGitProject() => _gitOperations.refreshProject();
  ...
  Future<void> showGitDiff(GitProjectChange change) =>
      _gitOperations.showDiff(change);
  ...
  Future<void> refreshGitReview() => _gitOperations.refreshReview();
  ...
  Future<bool> stageGitChange(GitProjectChange change) => _gitOperations.run(
    () =>
  ...
  Future<bool> revertGitChange(GitProjectChange change) => _gitOperations.run(
  ...
  Future<bool> commitGitChanges(String message) => _gitOperations.run(
    () =>
  ...
  Future<bool> pushGitBranch() => _gitOperations.run(
  ...
  Future<bool> createGitPullRequest(String title) => _gitOperations.run(
  ...
  void clearRuntimeLogs() => _runtimeDiagnostics.clear();
  ...
  String buildRuntimeDiagnosticReport() => _runtimeDiagnostics.buildReport();
  ...
  /// 判断 App Server 与外部桥接是否描述同一个子线程，避免实时项重复显示。
  /// Compares App Server and bridge activities so the same child thread is not shown twice.
  ...
  /// 返回候选值中第一个可展示的非空文本，用于兼容协议字段别名。
  /// Returns the first non-empty displayable value while tolerating protocol aliases.
  ...
  /// 读取协作桥接项的父会话 ID，并兼容桥接宿主的字段别名。
  /// Reads the parent conversation ID from a bridge item, accepting aliases
  /// used by older or non-Dart bridge hosts.
  String? _collaborationParentThreadId(JsonMap item) {
    // Accept spellings emitted by different bridge hosts while keeping one
    // normalized parent ID for the session-scope check.
  ...
  /// 执行一次实际发送；外层 [sendPrompt] 负责串行化并发提交。
  /// Performs one submission; the public [sendPrompt] method serializes concurrent calls.
  ...
  Future<void> resumeThread(CodexThread thread) async {
    ...
    final localTurnDiff = previousThreadId == thread.id
        ? turnDiff
        : _persistedTurnDiffByThreadId[thread.id];
    ...
    try {
      ...
      if (failure != null) {
        ...
        if (_isActiveWriterConflict(lastError!)) {
          _setThreadWriterConflict(
            threads: retryThreads!,
            operation: ThreadWriterConflictOperation.archive,
          );
        } else {
          _add(
            TimelineKind.error,
  ...
  /// 将已批准的浏览器请求去重后交给工作区导航，避免重复打开同一页面。
  /// Delivers an approved browser request once to the workspace navigation callback.
  ...
  /// Recursively extracts a navigable browser URL from nested protocol payloads.
        ...
        _storeSubagentThreadView(
          view.copyWith(
            status: switch (outcome) {
              TurnCompletionOutcome.succeeded => 'completed',
              TurnCompletionOutcome.failed => 'failed',
              TurnCompletionOutcome.stopped => 'stopped',
              TurnCompletionOutcome.unknown => view.status,
            },
          ),
        );
        _scheduleSubagentRefresh(threadId);
  ...
  void _handleTurnCompleted(JsonMap params) {
    ...
    switch (completionOutcome) {
      case TurnCompletionOutcome.failed:
      ...
      case TurnCompletionOutcome.stopped:
        status = RuntimeStatus.ready;
  ...
  void _reconcileUnidentifiedCompletion(
    ...
    for (final threadId in completedIds) {
      ...
      _updateThreadStatus(
        threadId,
        completionOutcome == TurnCompletionOutcome.failed
            ? 'systemError'
            : 'idle',
      );
      _setCompletionReminder(
        threadId,
        visible: completionOutcome == TurnCompletionOutcome.succeeded,
      );
      _clearPendingApprovalsForThread(threadId);
    }
  }
  ...
  bool _isTerminalThreadStatus(String? status) =>
      _threadLifecycle.isTerminalStatus(status);

  bool _isRunningThreadStatus(String? status) =>
      _threadLifecycle.isRunningStatus(status);
  ...
  /// 将技能读取和协作生命周期提升为可持久化的时间线活动。
  /// Promotes skill reads and collaboration lifecycles to durable timeline activities.
  ...
  /// 按来源或子线程 ID 更新已有活动，确保状态变化原位刷新而不重复插入。
  /// Updates an activity by source or child-thread ID so lifecycle changes stay in one row.
  ...
  /// 查找活动来源；桥接记录缺少稳定来源时回退到关联线程匹配。
  /// Finds an activity by source, falling back to its linked thread for bridge records.
  ...
  String? _nonEmptyConfigString(Object? value) =>
      _configurationValues.nonEmptyString(value);
  ...
  String? _configurationOriginLabel(Object? rawOrigins, String field) =>
      _configurationValues.originLabel(rawOrigins, field);
  ...
  ) => _workspacePaths.restoreConfiguration(
    stored,
  ...
  ConversationHistorySnapshot _conversationHistorySnapshot() {
    ...
    final fileChangesByThreadId = <String, List<CodexFileChange>>{
      for (final entry in _persistedFileChangesByThreadId.entries)
        entry.key: List<CodexFileChange>.of(entry.value),
    };
  ...
  /// 将缓存的任务页面恢复到共享控制器状态。
  void _restoreThreadView(ThreadViewSnapshot snapshot) {
    _conversationViewRevision++;
    _entries
      ..clear()
      ..addAll(snapshot.entries);
  ...
  /// restored from a stale draft after the original file was deleted.
  bool _isClipboardTemporaryPath(String path) => RegExp(
    r'(^|[/\\])CodexDeskClipboard[/\\]clipboard-image-[^/\\]+$',
    caseSensitive: false,
  ).hasMatch(path);

  /// 判断时间线条目是否来自无法归属父会话的旧桥接格式。
  /// Returns whether an external bridge entry lacks a real linked thread and
  ...
  Future<bool> _isSystemTemporaryDirectory(String canonicalPath) =>
      _workspacePaths.isSystemTemporaryDirectory(canonicalPath);
