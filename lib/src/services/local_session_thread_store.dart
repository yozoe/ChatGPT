...
class LocalSessionThreadStore {
  ...
  final Map<String, _CachedThreads> _cachedThreads = {};
  ...
  Future<List<CodexThread>> _loadThreads(String workspace) async {
    ...
    try {
      ...
      _cachedThreads[workspace] = _CachedThreads(
        threads: result,
...
/// 保存工作区线程缓存及其加载时间。
/// Holds cached workspace threads together with their load time.
class _CachedThreads {
  /// 创建带有加载时间的缓存条目。
  /// Creates a cache entry with its load timestamp.
  const _CachedThreads({required this.threads, required this.loadedAt});
