...
- 可新建并保存多个工作区，左侧栏按创建顺序以项目节点展示，切换时顺序保持不变；当前项高亮，其他项可直接切换，完整路径通过 tooltip 查看，列表标题提供独立的“新建”和“管理”入口。每个工作区独立维护一个主目录、多个附加目录和本地历史，并自动恢复及清理失效路径，不向项目目录写入配置。新建、恢复或切换工作区后会自动连接运行时；客户端会在初始化握手中声明实验性 App Server 能力，以便通过 `runtimeWorkspaceRoots` 将附加目录授权给后续新任务。目录可随时增删，已有任务不受影响
- 点击项目菜单中的“编辑”会打开接近 Codex 桌面端的项目编辑器：可修改项目显示名称、查看主源文件夹和附加源文件夹、添加或移除附加文件夹，并通过“移除本地项目”移除当前项目记录（不会删除磁盘目录或历史缓存）；“保存”持久化项目名称和目录集合，“取消”只关闭编辑器
- 自动保存并恢复 macOS 主窗口的大小与位置；覆盖安装同一应用后仍会保留
- 左侧导航采用 Codex 风格的项目树：工作区以文件夹节点展示，任务作为当前项目下的文件缩进排列，置顶任务单独分组；选中任务使用低对比圆角背景，运行中的当前任务在行尾显示加载指示器，已完成任务显示蓝色信息点，错误任务显示红色叹号。悬停项目会显示更多按钮和新建任务按钮，并自动展示项目详情卡片；详情卡片可置顶项目并显示当前项目或本地缓存中的任务数量，置顶状态会持久化；点击更多显示置顶、编辑、创建永久工作树、归档聊天和移除项目菜单，点击新建任务按钮会清空当前任务并开始新任务，工作区完整路径收纳在 tooltip 中
...
- 运行中任务的最新用户消息支持“调整方向”：可点击消息操作打开编辑器，也可直接在仍保持可编辑的 Composer 中输入并按 Enter，通过 App Server `turn/steer` 发送到当前活动 turn，不会创建新的任务线程；运行中仅接受纯文本调整，附件、目标、计划模式和技能上下文入口会暂时禁用；停止按钮仍用于中断任务
- 输入区上方会在存在文件变更时显示紧凑统计提示条，例如“1 个文件已更改 +15 -20”；统计复用文件 Diff，无法取得 Diff 时显示 `+? -?`
...
- 任务完成后会在对话区显示文件变更摘要卡片，列出文件数与新增/删除行数，并通过“审核”打开只读代码审查窗口；悬停单个已编辑文件时会显示跟随窗口边界自适应的紧凑 Diff 预览，按 unified Diff hunk 展示实际行号，并在 Diff 异步补齐或更新时同步刷新；窗口支持按文件查看逐行 Diff、长行横向滚动和完整任务 Diff，不提供提交或推送操作。若 App Server 未携带文件级 Diff，应用会对未跟踪文件从 Git 工作区安全补齐；无法可靠取得 Diff 时显示未知统计，不伪装成 `+0 -0`
...
- 如果 App Server 暂时返回空线程列表，活跃列表会回退读取当前项目的本地 Codex session 元数据；回退读取只解析每个文件开头的有限元数据，并按工作区合并并缓存 10 秒，同时保留本地缓存中的当前活动线程作为恢复候选，服务端有结果时始终以服务端为准，归档列表不使用此回退
...
- 审批默认请求批准；可在“变更与审批”切换为帮我批准。帮我批准模式会直接允许命令、文件变更与额外权限请求，并在时间线留下记录。
