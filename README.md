...
- 项目列顶部的搜索图标会打开 Codex 风格的“搜索聊天”面板：可按任务标题、预览、所属项目或 Provider 搜索所有已加载项目的任务，选择后会先切换到所属项目再恢复任务；面板还提供新聊天、打开文件夹和文件变更搜索的快捷入口。常用任务仍可置顶，置顶状态按本地项目保存并在重启后恢复
...
- 对话时间线采用扁平消息样式并自动定位到最新内容，不使用补间滚动动画；首次切换任务时显示 Codex 风格历史加载页，加载完成后缓存最近 8 个已打开任务的完整时间线页面及其滚动位置；再次切换直接显示已保活页面，不播放平滑滚动或重新定位动作；超出上限的较早任务会重新读取历史。历史读取失败时不会显示前一任务内容。Codex 回复按 GitHub Flavored Markdown 渲染，支持标题、列表、加粗、链接、引用、表格与代码块，且正文仍可选择复制；文本选择由时间线外层统一处理，不创建会抢占滚轮的嵌套文字滚动区；用户输入、命令和系统记录保持纯文本。连续的历史命令、文件读取与工具调用会收纳为默认展开的 Codex 风格活动清单，点击标题可折叠，完整事件详情仍通过条目提示保留。运行中的命令会以一条可复制、可截断的“正在运行”状态行显示，完成后归入该活动清单；每个结束的任务会在时间线中保留“已处理 X 分钟 X 秒”耗时。非当前任务或未打开任务收到的迟到/后台事件只刷新任务列表，不会改变当前时间线、命令或计划。Enter 发送消息，Shift+Enter 换行。App Server 的命令输出增量等高频协议事件不会显示为对话消息；需要决策的审批、任务结果和恢复历史中的实际命令记录仍会保留
- 运行中任务会根据 App Server 的 `turn/plan/updated` 通知悬浮展示结构化步骤，区分待执行、进行中与已完成状态，并显示当前“第 N / M 步”；任务结束后自动收起
- 运行中任务的最新用户消息支持“调整方向”：可点击消息操作打开编辑器，也可直接在仍保持可编辑的 Composer 中输入并按 Enter，通过 App Server `turn/steer` 发送到当前活动 turn，不会创建新的任务线程；运行中仍可附加文件、图片、文件夹、技能、目标和计划上下文；停止按钮仍用于中断任务
