...
- 可新建并保存多个工作区，左侧栏按创建顺序以项目节点展示，切换时顺序保持不变；当前项高亮，其他项可直接切换，完整路径通过 tooltip 查看，列表标题提供独立的“新建”和“管理”入口。点击“新建”会打开 Codex 风格的创建项目弹窗，项目名称可选，源文件夹区域既可点击选择目录，也可直接拖入目录。每个项目拥有独立、稳定的任务归属，不以源目录的历史会话作为边界：新建项目总是从空任务列表开始，后续创建的任务只显示在所属项目中。每个工作区独立维护一个主目录、多个附加目录和本地历史，并自动恢复及清理失效路径，不向项目目录写入配置。升级至项目独立历史时会自动读取并迁移原目录索引的本地缓存，避免旧任务在重启后消失。新建、恢复或切换工作区后会自动连接运行时；客户端会在初始化握手中声明实验性 App Server 能力，以便通过 `runtimeWorkspaceRoots` 将附加目录授权给后续新任务。目录可随时增删，已有任务不受影响
...
- 左侧导航采用 Codex 风格的项目树：工作区以文件夹节点展示，任务作为各项目下的文件缩进排列，已访问项目的任务列表在切换项目后仍保持展开；置顶任务单独分组；选中任务使用低对比圆角背景，运行中的当前任务在行尾显示高对比加载指示器并可点击保持当前焦点（不会切换任务），刚创建但尚未进入服务端列表的任务也会先显示在侧栏，未查看的已完成任务显示蓝色完成提醒（打开该任务后自动清除），错误任务显示红色叹号。悬停项目会显示更多按钮和新建任务按钮，并自动展示项目详情卡片；详情卡片可置顶项目并显示当前项目或本地缓存中的任务数量，置顶状态会持久化；点击更多显示置顶、编辑、创建永久工作树、归档聊天和移除项目菜单，点击新建任务按钮会清空当前任务并开始新任务，工作区完整路径收纳在 tooltip 中
- 左右侧栏之间提供可拖拽的细分隔条：左侧任务栏和右侧环境栏可在最小/最大宽度范围内调整，悬停或拖动时使用主题强调色反馈；窄窗口继续自动收起右侧环境栏
- 对话时间线采用扁平消息样式并自动定位到最新内容，不使用补间滚动动画；首次切换任务时显示 Codex 风格历史加载页，加载完成后缓存最近 8 个已打开任务的完整时间线页面及其滚动位置；再次切换直接显示已保活页面，不播放平滑滚动或重新定位动作；超出上限的较早任务会重新读取历史。历史读取失败时不会显示前一任务内容。Codex 回复按 GitHub Flavored Markdown 渲染，支持标题、列表、加粗、链接、引用、表格与代码块，且正文仍可选择复制；文本选择由时间线外层统一处理，不创建会抢占滚轮的嵌套文字滚动区；用户输入、命令和系统记录保持纯文本。连续的历史命令、文件读取与工具调用会收纳为默认展开的 Codex 风格活动清单，点击标题可折叠，完整事件详情仍通过条目提示保留。Enter 发送消息，Shift+Enter 换行。App Server 的命令输出增量等协议级执行事件不会显示为对话消息；需要决策的审批、任务结果和恢复历史中的实际命令记录仍会保留
...
- 存在文件变更时，输入框顶缘会悬浮显示紧凑统计提示，例如“1 个文件已更改 +15 -20”；提示不占用独立布局高度，统计复用文件 Diff，无法取得 Diff 时显示 `+? -?`
- 底部输入区采用深色圆角工作台样式，集中展示任务状态、审批模式、模型与推理强度，并会随窗口宽度折叠次要工具项；随心输入、模型选择和推理强度控件不绘制额外边框，推理强度直接显示“默认”“高”“中”“低”等强度名称
...
- 右侧检查器采用 Codex 风格的“环境信息”圆角卡片：展示本地 Git 变更、当前分支、提交/推送、创建拉取请求和比较分支入口；文件变更仍以紧凑可展开行列出本次任务完整 Diff 和 AI 修改的文件
