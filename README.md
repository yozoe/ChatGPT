...
- 对话时间线采用扁平消息样式并自动滚至最新内容；首次切换任务时显示 Codex 风格历史加载页，加载完成后缓存最近 8 个已打开任务的内存页面及其滚动位置，再次切换可直接恢复原浏览位置，不播放平滑滚动动画；超出上限的较早任务会重新读取历史。历史读取失败时不会显示前一任务内容。Codex 回复按 GitHub Flavored Markdown 渲染，支持标题、列表、加粗、链接、引用、表格与代码块，且正文仍可选择复制；用户输入、命令和系统记录保持纯文本。连续的历史命令、文件读取与工具调用会收纳为默认展开的 Codex 风格活动清单，点击标题可折叠，完整事件详情仍通过条目提示保留。Enter 发送消息，Shift+Enter 换行。App Server 的命令输出增量等协议级执行事件不会显示为对话消息；需要决策的审批、任务结果和恢复历史中的实际命令记录仍会保留
