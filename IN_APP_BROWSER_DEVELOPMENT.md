...
本文档定义 Codex Desk 的 macOS 内置浏览器开发范围与验收标准。它不是已交付功能：当前设置中的“浏览器（待开发）”仍是占位入口，所有网页链接仍通过系统默认浏览器打开。
...
| 位置 | 当前行为 | 对浏览器开发的影响 |
| --- | --- | --- |
| `lib/src/presentation/settings/codex_workspace_settings_page_state.dart` | “浏览器（待开发）”没有点击实现 | 可在现有设置导航中接入独立内容页。 |
