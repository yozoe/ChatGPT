...
本文档定义 Codex Desk 的 macOS 内置浏览器开发范围与验收标准。阶段 0 的 WebView 技术验证已完成；当前已加入智能体按需唤起适配层和设置开关。设置中的“集成 > 浏览器”只管理能力说明和权限策略，不手动创建 WebView。由于当前 App Server 未公开完整 browser tool 协议，适配层只接受明确的浏览器事件形状，偏好、历史、下载和统一 Markdown 链接路由仍在后续阶段开发。
...
- 从智能体浏览器调用打开时，切换至浏览器工作区而不是覆盖会话状态；返回会话后，草稿、附件、滚动位置和运行中的任务保持不变。未来接入链接、历史或常驻入口后沿用同一承载层。
...
已交付受限的 App Server/browser 请求适配、设置总开关、单标签浏览器工作区、地址栏与基本导航。仅对可回复的 `browser/open`、`browser/navigate` 请求显示权限卡片；`item/started`、无 ID 的通知及 `computer-use` 活动只作状态展示，绝不据此加载网页。验收：设置页不直接创建 WebView；仅当智能体发出受支持的浏览器请求、功能已启用且用户已允许时，才在当前应用加载 HTTP/HTTPS 页面；会话与运行中任务不受切换影响。完整公开协议接入、链接位置偏好和统一链接路由仍待协议确认。
