...
- 多条待发送的“调整方向”现在以递进宽度展示：较早条目逐级收窄，最新条目保持最宽，右侧操作对齐形成清晰层级；队列限高滚动和逐条发送、删除行为不变。
- 移除全局 Material 点击水波纹：按钮、任务行、菜单项和其他点击面不再显示扩散动画，原有悬停、选中及键盘焦点反馈保持不变。
- 修复当前任务完成后“任务已就绪”和侧栏蓝点持续不消失的问题：时间线已经位于底部时会立即自动确认；用户停留在上方阅读时继续保留提醒，随后滚到底部再自动隐藏，且不会误确认后台任务。
